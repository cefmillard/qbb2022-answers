...
def compute_power_matrix():
    ...
    for i, prob in enumerate(prob_heads):
        for j,toss in enumerate(n_tosses):
            power_mat[i,j] = run_experiment(prob, toss)
    return(power_mat)

def run_experiment_correct(prob_heads, n_toss, n_iters = 100, seed = 389, correct_the_pvalues = True):
    '''
    Input: prob_heads, a float, the probability of a simulated coin toss returning heads
           n_toss, an integer, the number of coin tosses to simulate
           n_iters, an integer, the number of iterations for each simulation. default is 100
...
def compute_power_matrix_correct():
    n_tosses = numpy.array([10, 50, 100, 250, 500, 1000])
    prob_heads = numpy.around(numpy.arange(0.55, 1.05, 0.05), decimals=2)[::-1]
    power_mat = numpy.zeros((len(prob_heads), len(n_tosses)))
    for i, prob in enumerate(prob_heads):
        for j,toss in enumerate(n_tosses):
            power_mat[i,j] = run_experiment_correct(prob, toss)
    return(power_mat)

power1 = compute_power_matrix()
powercorrect = compute_power_matrix_correct()

#print(power1)

fig, ax = plt.subplots()
sns.heatmap(power1, ax=ax, vmin = 0, vmax = 1, yticklabels = (numpy.around(numpy.arange(0.55, 1.05, 0.05), decimals=2)[::-1]), xticklabels = (10, 50, 100, 250, 500, 1000), cmap = "viridis", linewidths = 0.5)
ax.set_xlabel("Number of Tosses")
ax.set_ylabel("Probabilitity of Heads")
ax.set_title("Statistical Power as a Function of the Number of Times a Coin \n is Flipped and the Probability of Flipping Heads")
plt.savefig('powerheatmap.png')
plt.close(fig)

fig, ax = plt.subplots()
sns.heatmap(powercorrect, ax=ax, vmin = 0, vmax = 1, yticklabels = (numpy.around(numpy.arange(0.55, 1.05, 0.05), decimals=2)[::-1]), xticklabels = (10, 50, 100, 250, 500, 1000), cmap = "viridis", linewidths = 0.5)
ax.set_xlabel("Number of Tosses")
ax.set_ylabel("Probabilitity of Heads")
ax.set_title("Statistical Power as a Function of the Number of Times a Coin \n is Flipped and the Probability of Flipping Heads")
